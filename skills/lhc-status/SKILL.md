---
name: lhc-status
description: Read-only snapshot of ~/.lhc state — runtime, recent plans/artifacts, notepad tail, and readiness per workflow. Does not implement.
pipeline: [lhc-status]
next-skill: none
handoff: none
---
...
<Use_When>
- The user asks "what's in `~/.lhc/`", "what plans do I have", or "what's the readiness".
- The user wants to know which LHC skill to run next given current state.
- Before starting a new workflow, to confirm there isn't already one in flight.
</Use_When>

<Do_Not_Use_When>
- The user has a concrete workflow request — run the matching LHC skill.
...
6. **Synthesize and recommend the next skill**

   Close the report with a one-line recommendation like:
